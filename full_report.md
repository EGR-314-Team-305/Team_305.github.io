...
When doing the research into datasheets, there are example circuits that have specific values for capacitors, inductors, resistors, etc. While it is tempting to use the circuit as-is, the datasheet provides equations to follow that result in those values. Issues with circuit planning can be addressed much earlier if these equations are used for the project's use case.
...
Along with making the subsystems could talk with each other, the team learned how to use an ESP32 that talked with the microcontroller via UART, and with an MQTT server via WiFi. The lesson learned here was to get parts of the code done in parts and verify the functionality before adding another part.
...
When choosing the different chips of the subsystem, the specific package for the surface mount version can make soldering more or less difficult. The team attempted to stick to SOIC packaging which was relatively straightforward to solder. However some chips came in a UQFN package that made soldering much more difficult, and several were destroyed in the process.
The soldering process can also be made much easier by using resources provided by the Peralta Lab. Of particular note is the solder paste, which can be used to attach smaller integrated chips, resistors, and capacitors. This also results in a much cleaner finished product with minimal electrical shorts.
